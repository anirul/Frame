...
/**
 * @class ObjVertex
 * @brief Vertex of an obj file, it contain point, normal and texture coordinate.
 */
struct ObjVertex {
    glm::vec3 point;
    glm::vec3 normal;
...
class ObjMesh {
   public:
    /**
     * @brief Constructor create an obj object from a list of vertex, indices and materials.
     * @param points: Vector of vertices.
     * @param indices: Vector of indices as int.
     * @param material: Material id (watch out this is an  internal material not a entity id type of
     * material!).
     */
    ObjMesh(std::vector<ObjVertex> points, std::vector<int> indices, int material)
...
/**
 * @class Obj
 * @brief The class that will open an obj file and store data from it on the disk.
 */
class Obj {
   public:
    /**
