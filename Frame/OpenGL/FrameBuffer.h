...
/**
 * @class FrameBuffer
 * @brief Frame buffer class is a packer around the frame buffer concept from OpenGL. This will be
 * derived from the bind interface as the frame buffer can be bound to the current context.
 */
class FrameBuffer : public BindInterface {
   ...
   public:
    /**
     * @brief From the bind interface this will bind the framebuffer to the current context the slot
     ...
     * buffer & the render buffer in case it is not yet!
     * @param render: The render buffer that will be attach to the frame buffer.
     */
    void AttachRender(const RenderBuffer& render) const;
    /**
     * @brief Attach a texture to the frame buffer, warning this will bind the frame buffer and the
     * texture in case it is not yet!
     * @param texture_id: Id of the texture.
     * @param frame_color_attachment: On which frame do you need the attachment to be made.
     * @param frame_texture_type: What kind of texture is it normal or cubemap element.
     * @param mipmap: Mipmap level.
     */
    void AttachTexture(
        unsigned int texture_id,
        const FrameColorAttachment frame_color_attachment = FrameColorAttachment::COLOR_ATTACHMENT0,
        const FrameTextureType frame_texture_type         = FrameTextureType::TEXTURE_2D,
        const int mipmap                                  = 0) const;
    /**
     * @brief Define an array of buffers into which outputs from the fragment shader data will be
     * written.
   ...
   public:
     ...
     * @return Frame Texture type in local system.
     */
    static FrameTextureType GetFrameTextureType(frame::proto::TextureFrame texture_frame);
   ...
   protected:
    // This was here from the error interface (not used anymore) but it is still used inside.
    const std::pair<bool, std::string> GetError() const;
    // Convert from the internal frame texture type to the OpenGL type.
    const int GetFrameTextureType(const FrameTextureType frame_texture_type) const;
