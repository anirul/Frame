...
class Program : public ProgramInterface {
   public:
    //! @brief Construcor create the program.
    Program();
   ...
   private:
    ...
    mutable std::map<std::string, proto::Uniform::UniformEnum> uniform_variable_map_ = {};
...
std::optional<std::unique_ptr<frame::ProgramInterface>> CreateProgram(
    std::istream& vertex_shader_code, std::istream& pixel_shader_code);
