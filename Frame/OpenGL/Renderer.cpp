...
void Renderer::RenderNode(EntityId node_id, EntityId material_id /* = NullId*/,
    ...
    if (node_matrix) {
        ...
        if (flags) {
            ...
            if (flags & proto::CleanBuffer::CLEAR_COLOR) gl_clear |= GL_COLOR_BUFFER_BIT;
            if (flags & proto::CleanBuffer::CLEAR_DEPTH) gl_clear |= GL_DEPTH_BUFFER_BIT;
            glClear(gl_clear);
        }
    }
...
void Renderer::RenderMesh(StaticMeshInterface* static_mesh,
    ...
    glDrawElements(GL_TRIANGLES,
                   static_cast<GLsizei>(static_mesh->GetIndexSize()) / sizeof(std::int32_t),
