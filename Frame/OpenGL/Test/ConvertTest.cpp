...
namespace test {
	...
	{	
		...
		EXPECT_EQ(1.0f, frame::proto::ParseUniform(uniform.uniform_float()));
	}
	...
	TEST_F(ConvertTest, ParseUniformVec2Test)
	{
		...
		EXPECT_EQ(
			glm::vec2(1.0f, 2.0f), 
			frame::proto::ParseUniform(uniform_vec2));
	}
	...
	TEST_F(ConvertTest, ParseUniformVec3Test)
	{
		...
		EXPECT_EQ(
			...
			frame::proto::ParseUniform(uniform_vec3));
	}
	...
	TEST_F(ConvertTest, ParseUniformVec4Test)
	{
		...
		EXPECT_EQ(
			...
			frame::proto::ParseUniform(uniform_vec4));
	}
	...
	TEST_F(ConvertTest, ParseUniformMat4Test)
	{
		...
		EXPECT_EQ(glm::mat4(1.0f), frame::proto::ParseUniform(uniform_mat4));
	}
	...
	TEST_F(ConvertTest, ParseUniformVecFloatsTest)
	{
		...
		frame::proto::ParseUniformVec<float>(
			name, 
			uniform_floats, 
			program_mock);
	}
	...
	TEST_F(ConvertTest, ParseUniformFromProtoEmptyTest)
	{
		...
		EXPECT_THROW(
			frame::proto::RegisterUniformFromProto(
				uniform, 
				uniform_mock, 
				program_mock), 
			std::runtime_error);
	...
	TEST_F(ConvertTest, ParseUniformEnumInvalidFromProtoTest)
	{
		...
		StrictMock<UniformMock> uniform_mock{};
		EXPECT_THROW(
			frame::proto::RegisterUniformEnumFromProto(
				"test",
				uniform_enum, 
				uniform_mock, 
				program_mock),
