...
glm::vec4 ParseUniform(const UniformVector4& uniform_vec4) {
    return { uniform_vec4.x(), uniform_vec4.y(), uniform_vec4.z(), uniform_vec4.w() };
}
...
glm::quat ParseUniform(const UniformQuaternion& uniform_quat) {
    return { uniform_quat.w(), uniform_quat.x(), uniform_quat.y(), uniform_quat.z() };
}

void RegisterUniformFromProto(const Uniform& uniform, const UniformInterface& uniform_interface,
    ...
    switch (uniform.value_oneof_case()) {
        ...
        case Uniform::kUniformEnum: {
            RegisterUniformEnumFromProto(uniform.name(), uniform.uniform_enum(), uniform_interface,
                                         program_interface);
            return;
        }
        case Uniform::kUniformVec2: {
            program_interface.Uniform(uniform.name(), ParseUniform(uniform.uniform_vec2()));
            return;
        }
        case Uniform::kUniformVec3: {
            program_interface.Uniform(uniform.name(), ParseUniform(uniform.uniform_vec3()));
            return;
        }
        case Uniform::kUniformVec4: {
            program_interface.Uniform(uniform.name(), ParseUniform(uniform.uniform_vec4()));
            return;
        }
        case Uniform::kUniformMat4: {
            program_interface.Uniform(uniform.name(), ParseUniform(uniform.uniform_mat4()));
            return;
        }
        ...
        case Uniform::kUniformFloats: {
            ParseUniformVec<float>(uniform.name(), uniform.uniform_floats(), program_interface);
            return;
        }
        case Uniform::kUniformVec2S: {
            ParseUniformVec<UniformVector2>(uniform.name(), uniform.uniform_vec2s(),
                                            program_interface);
            return;
        }
        case Uniform::kUniformVec3S: {
            ParseUniformVec<UniformVector3>(uniform.name(), uniform.uniform_vec3s(),
                                            program_interface);
            return;
        }
        case Uniform::kUniformVec4S: {
            ParseUniformVec<UniformVector4>(uniform.name(), uniform.uniform_vec4s(),
                                            program_interface);
            return;
        }
        case Uniform::kUniformMat4S: {
            ParseUniformVec<UniformMatrix4>(uniform.name(), uniform.uniform_mat4s(),
                                            program_interface);
            return;
        }
        default:
...
void RegisterUniformEnumFromProto(const std::string& name, const Uniform::UniformEnum& uniform_enum,
    ...
    switch (uniform_enum) {
        ...
        case Uniform::PROJECTION_INV_MAT4: {
            program_interface.Uniform(name, glm::inverse(uniform_interface.GetProjection()));
            break;
        }
        ...
        case Uniform::VIEW_INV_MAT4: {
            program_interface.Uniform(name, glm::inverse(uniform_interface.GetView()));
            break;
        }
        ...
        case Uniform::FLOAT_TIME_S: {
            ...
            program_interface.Uniform(name, static_cast<float>(uniform_interface.GetDeltaTime()));
            break;
        }
        default:
...
std::pair<std::uint32_t, std::uint32_t> ParseSizeInt(const Size size) {
    return std::make_pair<std::uint32_t, std::uint32_t>(static_cast<std::uint32_t>(size.x()),
