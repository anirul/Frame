...
/**
 * @class Camera
 * @brief This is an implementation of a camera class it is done in regard to
 * computer graphics.
 */
class Camera {
 public:
   ...
   * You can set the camera mode by using the set camera mode method. Note that
   * in Y_AXIS_ALIGNED_ARCBALL you won't be able to modify the up vector by the
   * SetUp() method!
   *
   * @param position: Position of the camera.
   * @param front: Direction the camera is facing to (normalized).
   * @param up: Direction of the up for the camera (normalized).
   * @param fov_degrees: Field of view (angle in degrees of the vertical field
   * of view).
  ...
  Camera(glm::vec3 position = {0.f, 0.f, 0.f},
         glm::vec3 front = {0.f, 0.f, -1.f}, glm::vec3 up = {0.f, 1.f, 0.f},
         float fov_degrees = 65.0f, float aspect_ratio = 16.0f / 9.0f,
         float near_clip = 0.1f, float far_clip = 1000.0f,
 ...
 public:
  ...
  /**
   * @brief Update the front vector (normalized) and update the whole camera
   * accordingly.
