...
namespace frame::common {
	...
	/**
	 * @class Draw
	 * @brief The draw class is the main class that will be used to draw the
	 *        level.
	 */
	...
	public:
		 ...
		 * @param device: A pointer to the current device (come from the
		 *        window).
		 */
		Draw(
			glm::uvec2 size,
			...
			:
			size_(size),
			draw_type_based_(DrawTypeEnum::PATH),
			path_(path),
			device_(device)
		{}
		/**
	...
	public:
		...
		/**
		 * @brief Get the event from the window interface and pass them to the
		 *        draw.
