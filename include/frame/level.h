...
/**
 * @class Level
 * @brief This is the level where everything is gathered and organized to be
 * rendered. It is deriving from LevelInterface so you could eventually create a
 * better structure (entity based component).
 */
class Level : public LevelInterface {
 public:
   ...
   */
  std::vector<std::pair<
      EntityId, std::tuple<EntityId, proto::SceneStaticMesh::RenderTimeEnum>>>
  ...
  /**
   * @brief Set the default camera name (used during loading as the camera is
   * loaded after).
  ...
  /**
   * @brief Set the default scene root name (used during loading as the root
   * node is not loaded in a deterministic order).
   * @param name: Name of the scene root.
   */
  ...
  /**
   * @brief Get the default camera id, using the name that was stored during
   * loading.
   * @return An id or an error.
   */
  ...
  void AddMeshMaterialId(
      EntityId node_id, EntityId material_id,
   ...
   * @return An enum type.
   */
  EntityTypeEnum GetEnumTypeFromId(EntityId id) const override {
    return id_enum_map_.at(id);
  }
  /**
   * @brief Get name.
   * @return Name.
 ...
 public:
   ...
   */
  void ReplaceMesh(std::unique_ptr<StaticMeshInterface>&& mesh,
