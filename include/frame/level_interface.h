 ...
 * description of a Scene and all the object you need to render. This include
 * the scene tree the materials the programs and the meshes (for now only static
 * meshes).
 */
class LevelInterface : public NameInterface {
 public:
   ...
   */
  virtual void ReplaceTexture(std::vector<std::uint8_t>&& vector,
                              glm::uvec2 size, std::uint8_t bytes_per_pixel,
                              EntityId id) = 0;
  /**
   ...
   */
  virtual void ReplaceMesh(std::unique_ptr<StaticMeshInterface>&& mesh,
                           EntityId id) = 0;
};
