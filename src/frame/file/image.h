...
/**
 * @class Image
 * @brief This class is the representation of an image that come (or go) to a
 * file on the HDD. The idea is to load an image and save it on the GPU using
 * the texture class.
 */
class Image : public ImageInterface {
 public:
  ...
  /**
   * @brief Constructor this will build a image from a file and image specs,
   * this will be use to load an image from the HDD.
   * @param size: Size of the image.
   * @param pixel_element_size: Size of one of the element in a pixel (BYTE,
   * SHORT, HALF, FLOAT).
   * @param pixel_element_structure: Structure of a pixel (R, RG, RGB, RGBA).
 ...
 public:
   ...
   * @warning Note that the class won't be the owner of the pointer so it won't
