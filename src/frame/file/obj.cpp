...
Obj::Obj(const std::filesystem::path& file_name) {
    ...
    if (!reader.ParseFromFile(total_path)) {
        ...
        throw std::runtime_error(fmt::format("Unknown error parsing file [{}].", total_path));
    }

    logger_->info("Opening OBJ File [{}].", total_path);
    if (!reader.Warning().empty()) {
        logger_->warn("Warning parsing file {}: {}", total_path, reader.Warning());
    }
    ...
    bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err,
                                file_name.string().c_str(), directory.string().c_str());
    if (!warn.empty()) {
            ...
            for (std::size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
                ...
                if (fv != 3) {
                    throw std::runtime_error(
                        fmt::format("The face should be 3 in size now {}.", fv));
                }
                ...
                // per-face material
                if (material_id) assert(material_id == shapes[s].mesh.material_ids[f]);
    ...
    for (const auto& material : materials) {
        ...
        obj_material.ambient_vec4     = glm::vec4(material.ambient[0], material.ambient[1],
                                                  material.ambient[2], material.dissolve);
        obj_material.diffuse_str      = material.diffuse_texname;
        obj_material.diffuse_vec4     = glm::vec4(material.diffuse[0], material.diffuse[1],
