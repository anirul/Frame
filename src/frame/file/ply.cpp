...
void GetElementFloat(happly::PLYData& ply,
                     const std::array<std::string, SIZE>& names,
                     std::vector<std::vector<float>>& result) {
  if (result.size() != SIZE)
    throw std::runtime_error(fmt::format(
        "Result size is incorrect should be {} is {}", SIZE, result.size()));
...
std::vector<glm::vec3> GetElementVertexPropertyVec3(
  ...
  for (int i = 0; i < float_vec[0].size(); ++i) {
    result.emplace_back(
        glm::vec3(float_vec[0][i], float_vec[1][i], float_vec[2][i]));
  }
  return result;
}
...
Ply::Ply(const std::filesystem::path& file_name) {
  ...
  if (colors_.empty())
    colors_ = GetElementVertexPropertyVec3(ply_in, {"red", "green", "blue"});
  ...
  // Create a fake indices from 0 to the length of vertices in case there is no
  // indices.
  if (indices_.empty()) {
