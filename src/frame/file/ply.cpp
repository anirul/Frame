...
template <typename T>
void GetElementInternal(happly::PLYData& ply, const std::string& name, int i,
...
template <>
void GetElementInternal<std::uint8_t>(happly::PLYData& ply,
                                      const std::string& name, int i,
                                      std::vector<std::vector<float>>& result) {
    if (ply.getElement("vertex").hasPropertyType<std::uint8_t>(name)) {
        try {
            auto vec = ply.getElement("vertex").getProperty<std::uint8_t>(name);
            result[i].resize(vec.size());
            std::transform(vec.begin(), vec.end(), result[i].begin(),
                           [](std::uint8_t uc) { return static_cast<float>(uc) / 255.0f; });
        } catch (const std::exception& e) {
            Logger::GetInstance()->warn(e.what());
        }
    }
...
void GetElementFloat(happly::PLYData& ply, const std::array<std::string, SIZE>& names,
                     std::vector<std::vector<float>>& result) {
    if (result.size() != SIZE)
        throw std::runtime_error(
            fmt::format("Result size is incorrect should be {} is {}", SIZE, result.size()));
...
std::vector<glm::vec2> GetElementVertexPropertyVec2(happly::PLYData& ply,
    ...
    for (int i = 0; i < float_vec[0].size(); ++i) {
        result.emplace_back(glm::vec2(float_vec[0][i], float_vec[1][i]));
    }
    return result;
}

std::vector<glm::vec3> GetElementVertexPropertyVec3(happly::PLYData& ply,
    ...
    for (int i = 0; i < float_vec[0].size(); ++i) {
        result.emplace_back(glm::vec3(float_vec[0][i], float_vec[1][i], float_vec[2][i]));
    }
    return result;
}

std::vector<glm::vec4> GetElementVertexPropertyVec4(happly::PLYData& ply,
    ...
    for (int i = 0; i < float_vec[0].size(); ++i) {
        result.emplace_back(
            glm::vec4(float_vec[0][i], float_vec[1][i], float_vec[2][i], float_vec[3][i]));
    }
    return result;
}
...
Ply::Ply(const std::filesystem::path& file_name) {
    ...
    if (colors_.empty()) colors_ = GetElementVertexPropertyVec3(ply_in, { "red", "green", "blue" });
    normals_             = GetElementVertexPropertyVec3(ply_in, { "nx", "ny", "nz" });
    texture_coordinates_ = GetElementVertexPropertyVec2(ply_in, { "u", "v" });
    if (texture_coordinates_.empty())
        texture_coordinates_ = GetElementVertexPropertyVec2(ply_in, { "s", "t" });
    ...
    // Create a fake indices from 0 to the length of vertices in case there is no indices.
    if (indices_.empty()) {
