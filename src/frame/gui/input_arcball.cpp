...
namespace frame::gui {
    ...
    bool InputArcball::MouseMoved(glm::vec2 position, glm::vec2 relative,
        double dt) {
        ...
        glm::vec4 cam_front =
            glm::normalize(glm::vec4(camera.GetFront(), 1.0f));
    ...
    bool InputArcball::WheelMoved(float relative, double dt) {
        auto& camera = device_.GetLevel().GetDefaultCamera();
        float new_fov =
            camera.GetFovDegrees() + relative * zoom_multiplication_;
