...
namespace frame::gui {
    ...
    bool InputWasdMouse::MouseMoved(glm::vec2 position, glm::vec2 relative,
        double dt) {
        if (!mouse_button_pressed_) return true;
        ...
        if (mouse_button_pressed_ == 19) {
            const float inc =
                rotation_multiplication_ * static_cast<float>(dt);
            if (relative.x) {
        ...
        // Right button = translation.
            ...
            const float inc =
                translation_multiplication_ * static_cast<float>(dt);
            auto position = camera.GetPosition();
            if (relative.x) {
