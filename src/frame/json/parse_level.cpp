...
std::unique_ptr<LevelInterface> LevelProto(glm::uvec2 size,
  ...
  for (const auto& proto_texture : proto_level.textures()) {
    ...
    logger->info(fmt::format("Add a new texture {}, with id [{}].",
                             proto_texture.name(), texture_id));
    if (!texture_id) {
      throw std::runtime_error(fmt::format("Coudn't save texture {} to level.",
                                           proto_texture.name()));
    }
  }
  ...
  for (const auto& proto_program : proto_level.programs()) {
    ...
    if (!level->AddProgram(std::move(program))) {
      throw std::runtime_error(fmt::format("Couldn't save program {} to level.",
                                           proto_program.name()));
    }
  }
...
std::unique_ptr<LevelInterface> ParseLevel(glm::uvec2 size,
                                           const proto::Level& proto_level) {
  return LevelProto(size, proto_level);
}

std::unique_ptr<frame::LevelInterface> ParseLevel(glm::uvec2 size,
