...
[[nodiscard]] bool ParseSceneMatrix(LevelInterface& level,
  ...
  if (proto_scene_matrix.has_matrix()) {
    ...
    scene_matrix =
        std::make_unique<NodeMatrix>(GetFunctor(level), glm::mat4(1.0f));
  }
...
[[nodiscard]] bool ParseSceneStaticMeshFileName(
  ...
  auto vec_node_mesh_id = opengl::file::LoadStaticMeshesFromFile(
      level, "asset/model/" + proto_scene_static_mesh.file_name(),
      proto_scene_static_mesh.name(), proto_scene_static_mesh.material_name());
...
[[nodiscard]] bool ParseSceneStaticMeshStreamInput(
  ...
  auto point_buffer =
      std::make_unique<opengl::Buffer>(opengl::BufferTypeEnum::ARRAY_BUFFER,
...
[[nodiscard]] bool ParseSceneCamera(
  ...
  std::unique_ptr<NodeInterface> scene_camera = std::make_unique<NodeCamera>(
      ...
      ParseUniform(proto_scene_camera.up()), proto_scene_camera.fov_degrees(),
      proto_scene_camera.aspect_ratio(), proto_scene_camera.near_clip(),
...
[[nodiscard]] bool ParseSceneLight(LevelInterface& level,
  ...
  switch (proto_scene_light.light_type()) {
    case proto::SceneLight::POINT: {
      std::unique_ptr<NodeInterface> node_light =
          std::make_unique<frame::NodeLight>(
              GetFunctor(level), NodeLightEnum::POINT,
    ...
    case proto::SceneLight::DIRECTIONAL: {
      std::unique_ptr<NodeInterface> node_light =
          std::make_unique<frame::NodeLight>(
              GetFunctor(level), NodeLightEnum::DIRECTIONAL,
    ...
    default:
      throw std::runtime_error(
          fmt::format("Unknown scene light type {}",
