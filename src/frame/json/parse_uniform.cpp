...
glm::mat4 ParseUniform(const UniformMatrix4& uniform_mat4) {
  return glm::mat4(uniform_mat4.m11(), uniform_mat4.m12(), uniform_mat4.m13(),
                   uniform_mat4.m14(), uniform_mat4.m21(), uniform_mat4.m22(),
                   uniform_mat4.m23(), uniform_mat4.m24(), uniform_mat4.m31(),
                   uniform_mat4.m32(), uniform_mat4.m33(), uniform_mat4.m34(),
                   uniform_mat4.m41(), uniform_mat4.m42(), uniform_mat4.m43(),
                   uniform_mat4.m44());
}

glm::quat ParseUniform(const UniformQuaternion& uniform_quat) {
  return {uniform_quat.w(), uniform_quat.x(), uniform_quat.y(),
          uniform_quat.z()};
}

void RegisterUniformFromProto(const Uniform& uniform,
  ...
  switch (uniform.value_oneof_case()) {
    ...
    case Uniform::kUniformEnum: {
      RegisterUniformEnumFromProto(uniform.name(), uniform.uniform_enum(),
                                   uniform_interface, program_interface);
      return;
    }
    case Uniform::kUniformVec2: {
      program_interface.Uniform(uniform.name(),
    ...
    case Uniform::kUniformFloats: {
      ParseUniformVec<float>(uniform.name(), uniform.uniform_floats(),
                             program_interface);
      return;
    }
