...
EntityId Level::AddSceneNode(const std::string& name, std::shared_ptr<NodeInterface> scene_node) {
...
EntityId Level::AddTexture(const std::string& name, std::shared_ptr<TextureInterface> texture) {
    if (string_set_.count(name)) throw std::runtime_error("Name: " + name + " is already in!");
...
EntityId Level::AddProgram(const std::string& name, std::shared_ptr<ProgramInterface> program) {
    if (string_set_.count(name)) throw std::runtime_error("Name: " + name + " is already in!");
...
EntityId Level::AddMaterial(const std::string& name, std::shared_ptr<MaterialInterface> material) {
    if (string_set_.count(name)) throw std::runtime_error("Name: " + name + " is already in!");
...
EntityId Level::AddBuffer(const std::string& name, std::shared_ptr<BufferInterface> buffer) {
    if (string_set_.count(name)) throw std::runtime_error("Name: " + name + " is already in!");
...
EntityId Level::AddStaticMesh(const std::string& name,
