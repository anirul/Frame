...
class NodeCamera : public NodeInterface {
 public:
   ...
   */
  NodeCamera(std::function<NodeInterface*(const std::string&)> func,
             ...
             const float near_clip = 0.1f, const float far_clip = 1000.0f)
      : NodeInterface(func),
        camera_(std::make_unique<Camera>(position, target, up, fov_degrees,
