...
class NodeLight : public NodeInterface {
 public:
  ...
  NodeLight(std::function<NodeInterface*(const std::string&)> func,
      ...
      : NodeInterface(func),
        light_type_(NodeLightEnum::AMBIENT),
        color_(color) {}
  /**
   ...
   * @param position_or_direction: Position (if point light) or direction (if
   * directional light).
   * @param color: Color of the light in vec3 format.
   */
  NodeLight(std::function<NodeInterface*(const std::string&)> func,
            const frame::NodeLightEnum light_type,
            const glm::vec3 position_or_direction, const glm::vec3 color);
  /**
  ...
  NodeLight(std::function<NodeInterface*(const std::string&)> func,
            ...
            const glm::vec3 color, const float dot_inner_limit,
