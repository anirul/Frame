...
/**
 * @class NodeMatrix
 * @brief Node model of a matrix this is the basic step in the scene tree all
 * root should be of matrix type.
 */
class NodeMatrix : public NodeInterface {
 public:
   ...
   * @param matrix: A matrix in mat4 format that represent a transform for this
   * point.
   */
  NodeMatrix(std::function<NodeInterface*(const std::string&)> func,
             glm::mat4 matrix)
      : NodeInterface(func), matrix_(matrix) {}
  /**
   * @brief Constructor with a function and a quaternion quat entry. This will
   * enable rotation on a perpetual cycle.
   * @param func: This function return the ID from a string (it will need a
   * level passed in the capture list).
   * @param quat: quaternion representing a rotation to this node (note this
   * will be transfered to a mat4 at creation).
   */
  NodeMatrix(std::function<NodeInterface*(const std::string&)> func,
             glm::quat quat)
      : NodeInterface(func),
  ...
  /**
   * @brief Constructor with a quaternion quat entry. This will enable rotation
