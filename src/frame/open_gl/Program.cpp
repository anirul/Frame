...
std::optional<std::unique_ptr<ProgramInterface>> CreateProgram(std::istream& vertex_shader_code,
...
void Program::Uniform(const std::string& name, const proto::Uniform::UniformEnum enum_value) const {
...
const std::vector<EntityId> Program::GetInputTextureIds() const { return input_texture_ids_; }
...
const std::vector<EntityId> Program::GetOutputTextureIds() const { return output_texture_ids_; }
...
const std::vector<std::string>& Program::GetUniformNameList() const {
    ...
    for (GLuint i = 0; i < static_cast<GLuint>(count); ++i) {
        ...
        std::string warning_str =
            fmt::format("Program[{}].[{}]<{}>({})", program_id_, name_str, type, size);
