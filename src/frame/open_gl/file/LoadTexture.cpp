...
std::unique_ptr<frame::TextureInterface> LoadTextureFromFile(
    ...
    return std::make_unique<frame::opengl::Texture>(image.GetSize(), image.Data(),
...
std::unique_ptr<frame::TextureInterface> LoadCubeMapTextureFromFile(
    ...
    auto maybe_level = frame::proto::ParseLevelOpenGL(
        cube_pair_res,
        proto::LoadProtoFromJson<proto::Level>(FillLevel(proto_level_json, filling_map)));
    if (!maybe_level) {
    ...
    if (!maybe_camera_boon_id) throw std::runtime_error("Could not get camera boon id.");
    ...
    if (!scene_matrix) throw std::runtime_error("Could not cast to NodeMatrix.");
...
std::unique_ptr<frame::TextureInterface> LoadCubeMapTextureFromFiles(
    ...
    return std::make_unique<opengl::TextureCubeMap>(img_size, pointers, pixel_element_size,
                                                    pixel_structure);
}
...
std::unique_ptr<TextureInterface> LoadTextureFromVec4(const glm::vec4& vec4) {
    ...
    return std::make_unique<frame::opengl::Texture>(
        std::make_pair<std::uint32_t, std::uint32_t>(1, 1), ar.data(),
...
std::unique_ptr<TextureInterface> LoadTextureFromFloat(float f) {
    return std::make_unique<frame::opengl::Texture>(
        std::make_pair<std::uint32_t, std::uint32_t>(1, 1), &f,
        frame::proto::PixelElementSize_FLOAT(), frame::proto::PixelStructure_GREY());
}
