...
class Buffer : public BindInterface, public BufferInterface {
 public:
  /**
   * @brief Constructor this is where you can define the way a buffer is handled
   * on the code level.
   * @param buffer_type: This is the type of buffer you want to use
   * (ARRAY_BUFFER should be enough for most cases).
   * @param buffer_usage: Access way of the buffer is it gonna change or is it
   * going to be the same for a large amount of time.
   */
 ...
 public:
  ...
  /**
   * @brief From the bind interface this is where we unbind the buffer from the
   * current context.
   */
 ...
 public:
  /**
   * @brief From the bind interface that lock the bind interface to the current
   * context, this is used by the scoped bind interface for more RAII.
   */
  void LockedBind() const override { locked_bind_ = true; }
  /**
   * @brief Same as before this is the unlock part suppose to be called by the
   * scoped lock interface.
   */
  void UnlockedBind() const override { locked_bind_ = false; }
  /**
   * @brief From the bind interface this is suppose to return the object counter
   * that allow OpenGL to manage object.
   */
