...
/**
 * @brief This is the main buffer class, this is here so hold of a single dimensional element (see
...
class Buffer : public BindInterface, public BufferInterface {
   public:
     ...
     * same for a large amount of time.
     */
   ...
   public:
    ...
    /**
     * @brief From the bind interface this is where we unbind the buffer from the current context.
     */
    void UnBind() const override;

   public:
    /**
     * @brief From the bind interface that lock the bind interface to the current context, this is
     * used by the scoped bind interface for more RAII.
     */
    void LockedBind() const override { locked_bind_ = true; }
    /**
     * @brief Same as before this is the unlock part suppose to be called by the scoped lock
     * interface.
     */
    void UnlockedBind() const override { locked_bind_ = false; }
    /**
     * @brief From the bind interface this is suppose to return the object counter that allow OpenGL
     * to manage object.
     */
 ...
 * @param vector: A vector that is moved into the device and level.
 */
std::unique_ptr<BufferInterface> CreateIndexBuffer(std::vector<std::uint32_t>&& vector);
