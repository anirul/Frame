...
void Device::Startup(std::unique_ptr<frame::LevelInterface>&& level) {
  ...
  camera.SetAspectRatio(static_cast<float>(size_.x) /
                        static_cast<float>(size_.y));
  // Create a renderer.
  renderer_ = std::make_unique<Renderer>(*level_.get(),
                                         glm::uvec4(0, 0, size_.x, size_.y));
...
void Device::DisplayCamera(const Camera& camera, glm::uvec4 viewport,
                           double time) {
  renderer_->SetViewport(viewport);
  renderer_->RenderAllMeshes(camera.ComputeProjection(), camera.ComputeView(),
                             time);
}

void Device::DisplayLeftRightCamera(const Camera& camera_left,
...
void Device::Display(double dt /*= 0.0*/) {
  ...
  left_camera.SetPosition(left_camera.GetPosition() -
                          left_camera.GetRight() * interocular_distance_ *
                              0.5f);
  ...
  right_camera.SetPosition(right_camera.GetPosition() +
                           right_camera.GetRight() * interocular_distance_ *
                               0.5f);
  ...
  switch (stereo_enum_) {
    ...
    case StereoEnum::HORIZONTAL_SIDE_BY_SIDE:
      ...
      break;
    default:
      throw std::runtime_error(fmt::format("Unknown StereoEnum type {}.",
                                           static_cast<int>(stereo_enum_)));
  }
  // Reset viewport.
...
void Device::ScreenShot(const std::string& file) const {
  ...
  file::Image output_image(texture.GetSize(), pixel_element_size,
                           pixel_structure);
...
std::unique_ptr<frame::StaticMeshInterface> Device::CreateStaticMesh(
  ...
  return std::make_unique<opengl::StaticMesh>(GetLevel(),
                                              static_mesh_parameter);
}
...
void Device::SetStereo(StereoEnum stereo_enum, float interocular_distance,
                       glm::vec3 focus_point, bool invert_left_right) {
