...
class Device : public DeviceInterface {
 ...
 public:
  /**
   * @brief Set the stereo mode (by default this is NONE), interocular distance
   * and focus point.
   * @param stereo_enum: Set the mode the stereo will use.
   * @param interocular_distance: Distance between the eyes.
   * @param focus_point: Point of focus in the 3D scene (if 0 then they will
   * look in parallel).
   */
   ...
   * @param file: File name of the screenshot (usually with the *.png) extension
   * it will be dropped at the path where the software is run.
   */
 ...
 public:
   ...
   * @return A pointer to the current context (this is used by the windowing
   * system).
   */
 ...
 protected:
  ...
  void DisplayLeftRightCamera(const Camera& camera_left,
