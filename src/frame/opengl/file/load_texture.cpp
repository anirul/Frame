...
std::string FillLevel(const std::string& initial,
...
std::unique_ptr<frame::TextureInterface> LoadCubeMapTextureFromFile(
  ...
  for (std::uint32_t i = 0; i < 6; ++i) {
    ...
    renderer.RenderMesh(mesh_ref, material_ref, projection_cubemap,
                        views_cubemap[i]);
  }
  // FIXME(anirul): Why?
...
std::unique_ptr<TextureInterface> LoadTextureFromVec4(const glm::vec4& vec4) {
  ...
  TextureParameter texture_parameter = {proto::PixelElementSize_FLOAT(),
