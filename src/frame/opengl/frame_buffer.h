...
/**
 * @class FrameBuffer
 * @brief Frame buffer class is a packer around the frame buffer concept from
 * OpenGL. This will be derived from the bind interface as the frame buffer can
...
class FrameBuffer : public BindInterface {
 ...
 public:
   ...
   * current context the slot parameter can be forgotten as it has no sens with
   * frame buffers.
  ...
  /**
   * @brief Attach a render buffer to the current frame buffer, warning this
   * will bind the frame buffer & the render buffer in case it is not yet!
   * @param render: The render buffer that will be attach to the frame buffer.
   */
  void AttachRender(const RenderBuffer& render) const;
  /**
   * @brief Attach a texture to the frame buffer, warning this will bind the
   * frame buffer and the texture in case it is not yet!
   * @param texture_id: Id of the texture.
   * @param frame_color_attachment: On which frame do you need the attachment to
   * be made.
   * @param frame_texture_type: What kind of texture is it normal or cubemap
   * element.
  ...
  /**
   * @brief Define an array of buffers into which outputs from the fragment
   * shader data will be written.
 ...
 protected:
  // This was here from the error interface (not used anymore) but it is still
  // used inside.
