...
static bool ImGui_ImplSDL2_Init(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    (void)window;
        ...
        // Set SDL hint to receive mouse click events on window focus, otherwise
        // SDL doesn't emit the event. Without this, when clicking to gain
        // focus, our widgets wouldn't activate even though they showed as
        // hovered. (This is unfortunately a global SDL setting, so enabling it
        // might have a side-effect on your application. It is unlikely to make
        // a difference, but if your app absolutely needs to ignore the initial
        // on-focus click: you can ignore SDL_MOUSEBUTTONDOWN events coming
...
#define MAP_ANALOG(KEY_NO, AXIS_NO, V0, V1)                                     \
    ...
    MAP_BUTTON(
        ImGuiKey_GamepadFaceDown, SDL_CONTROLLER_BUTTON_A); // Xbox A, PS Cross
    MAP_BUTTON(
        ImGuiKey_GamepadFaceRight,
        SDL_CONTROLLER_BUTTON_B); // Xbox B, PS Circle
    MAP_BUTTON(
        ImGuiKey_GamepadFaceLeft, SDL_CONTROLLER_BUTTON_X); // Xbox X, PS Square
    MAP_BUTTON(
        ImGuiKey_GamepadFaceUp, SDL_CONTROLLER_BUTTON_Y); // Xbox Y, PS Triangle
