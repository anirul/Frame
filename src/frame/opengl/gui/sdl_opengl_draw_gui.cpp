...
bool SDL2OpenGLDrawGui::Update(DeviceInterface& device, double dt) {
  ...
  for (const EntityId& id : device.GetLevel().GetAllTextures()) {
    ...
    // If the window are not visible and it is not the main window then bail
    // out.
    if (!is_visible_ && !is_default_output) continue;
    if (!is_visible_ && is_default_output) {
      ImGui::Begin(fmt::format("{} - <fullscreen>", texture.GetName()).c_str(),
    ...
    // I disable the warning C4312 from unsigned int to void* casting to a
    // bigger space.
...
bool SDL2OpenGLDrawGui::PollEvent(void* event) {
  ...
  // Process the event in ImGui (has to be done or you won't be able to handle
  // the main window).
