...
/**
 * @class LightPoint
 * @brief Derived from light interface, light point is a point of light in a scene. It radiate in
 * all direction equally.
 */
class LightPoint : public LightInterface {
   public:
     ...
     * @param color_intensity: Color of the light point multiplied by intensity
     */
    LightPoint(const glm::vec3 position, const glm::vec3 color_intensity)
   ...
   public:
     ...
     */
    const LightTypeEnum GetType() const override { return LightTypeEnum::POINT_LIGHT; }
    /**
     * @brief Get the position of the light, coming from the light interface.
     ...
     */
    const glm::vec3 GetColorIntensity() const override { return color_intensity_; }
 ...
 * @brief Derived from light interface, This is a light that come from an infinite position and
 * follow a strait line (like a theoretical sun).
 */
class LightDirectional : public LightInterface {
   public:
     ...
     * @param color_intensity: Color of the light (technically not containing any intensity).
     */
    LightDirectional(const glm::vec3 direction, const glm::vec3 color_intensity)
   ...
   public:
     ...
     * @return Return the type of the light.
     */
    const LightTypeEnum GetType() const override { return LightTypeEnum::DIRECTIONAL_LIGHT; }
    /**
     * @brief Get the position of the light, coming from the light interface.
     * @return Return the position of a light.
     */
...
/**
 * @class LightManager
 * @brief This is where the light are gathered and assembled to be send to a program.
 */
class LightManager {
   public:
    /**
     ...
     */
    void AddLight(std::unique_ptr<LightInterface>&& light) { lights_.push_back(std::move(light)); }
     ...
     */
    const LightInterface* GetLight(int i) const { return lights_.at(i).get(); }
