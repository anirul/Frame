...
void Program::LinkShader() {
  ...
  if (error != GL_NO_ERROR) {
    std::string error_str =
        fmt::format("Failed to link program [{}], ",
...
void Program::Uniform(const std::string& name, const std::vector<float>& vector,
  ...
  if (size.y == 0 && size.x == 0) {
    ...
    throw std::runtime_error(fmt::format(
        "Unknown size doesn't know that size equivalent: {}", vector.size()));
  }
  ...
  if (size.y == 2) {
    ...
    if (size.x == 2) {
      glUniformMatrix2fv(GetMemoizeUniformLocation(name), 1, GL_FALSE,
                         vector.data());
      return;
    }
  }
  if (size.y == 3) {
    if (size.x == 1) {
      glUniform3f(GetMemoizeUniformLocation(name), vector[0], vector[1],
                  vector[2]);
      return;
    }
    if (size.x == 3) {
      glUniformMatrix3fv(GetMemoizeUniformLocation(name), 1, GL_FALSE,
                         vector.data());
      return;
    }
  }
  if (size.y == 4) {
    if (size.x == 1) {
      glUniform4f(GetMemoizeUniformLocation(name), vector[0], vector[1],
                  vector[2], vector[3]);
      return;
    }
    if (size.x == 4) {
      glUniformMatrix4fv(GetMemoizeUniformLocation(name), 1, GL_FALSE,
                         vector.data());
      return;
    }
  }
  throw std::runtime_error(
      fmt::format("Unknown size doesn't know that size equivalent: < {}, {} >.",
                  size.x, size.y));
}
...
void Program::Uniform(const std::string& name,
  ...
  if (size.y == 3) {
    if (size.x == 1) {
      glUniform3i(GetMemoizeUniformLocation(name), vector[0], vector[1],
                  vector[2]);
      return;
    }
  }
  if (size.y == 4) {
    if (size.x == 1) {
      glUniform4i(GetMemoizeUniformLocation(name), vector[0], vector[1],
                  vector[2], vector[3]);
      return;
    }
  }
...
int Program::GetMemoizeUniformLocation(const std::string& name) const {
  if (!memoize_map_.count(name)) {
    ...
    if (location == -1) {
      GLenum error = glGetError();
      throw std::runtime_error(fmt::format(
          "Could not get a location for uniform [{}] error: {}.", name,
          reinterpret_cast<const char*>(gluErrorString(error))));
    }
...
void Program::ThrowIsInTextureIds(EntityId texture_id) const {
  if (std::count(input_texture_ids_.begin(), input_texture_ids_.end(),
                 texture_id)) {
    throw std::runtime_error("Texture: [" + std::to_string(texture_id) +
                             " is already in input texture ids.");
  }
  if (std::count(output_texture_ids_.begin(), output_texture_ids_.end(),
                 texture_id)) {
    throw std::runtime_error("Texture: [" + std::to_string(texture_id) +
...
std::unique_ptr<ProgramInterface> CreateProgram(
    const std::string& name, std::istream& vertex_shader_code,
