...
void Renderer::RenderNode(EntityId node_id, EntityId material_id,
  ...
  RenderMesh(static_mesh, material, projection, view, node.GetLocalModel(dt),
             dt);
}
...
void Renderer::RenderMesh(StaticMeshInterface& static_mesh,
  ...
  for (const auto& texture_id : program.GetOutputTextureIds()) {
    if (level_.GetTextureFromId(texture_id).IsCubeMap()) {
      ...
      frame_buffer_.AttachTexture(
          opengl_texture.GetId(), FrameBuffer::GetFrameColorAttachment(i),
  ...
  for (const auto& id : material.GetIds()) {
    ...
    if (level_.GetEnumTypeFromId(id) == EntityTypeEnum::TEXTURE) {
      ...
      // TODO(anirul): Find a better way to find the texture associated with the
      // stream.
      texture_id = id + 1;
    }
    ...
    switch (static_mesh.GetRenderPrimitive()) {
      ...
      default:
        throw std::runtime_error(
            fmt::format("Couldn't draw primitive {}",
...
void Renderer::RenderAllMeshes(const glm::mat4& projection,
    ...
    if (render_time_enum == proto::SceneStaticMesh::PRE_RENDER) {
      ...
      if (first_render) {
        ...
        for (std::uint32_t i = 0; i < 6; ++i) {
          ...
          RenderNode(p.first, material_id, projection_cubemap, views_cubemap[i],
                     dt);
        }
        // Again why?
        SetCubeMapTarget(GetTextureFrameFromPosition(0));
        RenderNode(p.first, material_id, projection_cubemap, views_cubemap[0],
                   dt);
      }
