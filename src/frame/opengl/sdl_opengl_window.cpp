...
namespace frame::opengl {
    ...
    SDLOpenGLWindow::SDLOpenGLWindow(glm::uvec2 size) : size_(size) {
        ...
        sdl_window_ = SDL_CreateWindow("SDL OpenGL", SDL_WINDOWPOS_UNDEFINED,
            SDL_WINDOWPOS_UNDEFINED, size_.x, size_.y,
    ...
    void SDLOpenGLWindow::Run(std::function<void()> lambda) {
        ...
        do {
            ...
            SetWindowTitle("SDL OpenGL - " +
                std::to_string(static_cast<float>(GetFPS(dt))));
        ...
        if (input_interface_) {
            ...
            if (event.type == SDL_MOUSEMOTION) {
                return input_interface_->MouseMoved(
                    ...
                    glm::vec2(event.motion.xrel, event.motion.yrel), dt);
            }
    ...
    void* SDLOpenGLWindow::GetGraphicContext() const {
        ...
        logger->info("Started SDL OpenGL version {}.{}.", gl_version.first,
            gl_version.second);
    ...
    void SDLOpenGLWindow::Resize(
        glm::uvec2 size,
        FullScreenEnum fullscreen_enum)
    {
        size_ = size;
    ...
    namespace {
            ...
            LPARAM param)
        {
            ...
            float horz_scale =
                static_cast<float>(cx_physical) /
                static_cast<float>(cx_logical);
            float vert_scale =
                static_cast<float>(cy_physical) /
                static_cast<float>(cy_logical);
            ...
            s_ppi_vec.push_back(
                glm::vec2(static_cast<float>(hppi) * multiplication_scale,
    ...
    glm::vec2 SDLOpenGLWindow::GetPixelPerInch(
        std::uint32_t screen /*= 0*/) const
    {
#if defined(_WIN32) || defined(_WIN64)
        // Reset the vector.
        s_ppi_vec = {};
