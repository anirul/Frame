...
namespace frame::opengl {
    ...
    StaticMesh::StaticMesh(
        ...
        const StaticMeshParameter& parameter)
        : level_(level),
          ...
          render_primitive_enum_(parameter.render_primitive_enum)
    {
        ...
        if (color_buffer_id_) {
        ...
        else if (std::count(parameter.generate_list.begin(),
            parameter.generate_list.end(),
            StaticMeshParameter::StaticMeshParameterEnum::
            GENERATE_COLOR)) {
            ...
            gl_color_buffer->SetName(
                fmt::format("Mesh.Buffer.Color.{}", count));
        ...
        if (texture_buffer_id_) {
            auto& gl_texture_buffer =
                dynamic_cast<Buffer&>(
                    level.GetBufferFromId(texture_buffer_id_));
            ...
            std::fill(
                texture_coordinate.begin(),
                texture_coordinate.end(),
                0.5f);
            ...
            texture_buffer_id_ =
                level_.AddBuffer(std::move(gl_texture_coordinate));
            auto& texture_buffer_ref =
                dynamic_cast<Buffer&>(
                    level_.GetBufferFromId(texture_buffer_id_));
        ...
        if (!index_buffer_id_) {
            ...
            {
                throw std::runtime_error(
                    "No GENERATE_INDEX in the generate list.");
            }
            ...
            gl_index_buffer->SetName(
                fmt::format("Mesh.Buffer.Index.{}", count));
    ...
    EntityId CreateQuadStaticMesh(LevelInterface& level) {
        ...
        auto maybe_texture_buffer_id = level.AddBuffer(
