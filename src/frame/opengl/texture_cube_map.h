...
class TextureCubeMap : public TextureInterface, public BindInterface {
 ...
 public:
  ...
  //! @brief Enable mip map, this allow a recursive level of texture faster for
  //! rendering.
  void EnableMipmap() const override;
  /**
   * @brief From the bind interface this will bind the texture at slot to the
   * current context.
   * @param slot: Slot to be binded.
   */
  void Bind(const unsigned int slot = 0) const override;
  //! @brief From the bind interface this will unbind the current texture from
  //! the context.
  ...
  /**
   * @brief Set the wrapping on the s size of the texture (horizontal) this will
   * decide how the texture is treated in case you overflow in this direction.
   ...
   * @return The way the texture is wrap could be any of (REPEAT, CLAMP_TO_EDGE,
   * MIRRORED_REPEAT).
   */
  proto::TextureFilter::Enum GetWrapS() const override;
  /**
   * @brief Set the wrapping on the t size of the texture (vertical) this will
   * decide how the texture is treated in case you overflow in this direction.
   * @param texture_filter: Could be any of (REPEAT, CLAMP_TO_EDGE,
   * MIRRORED_REPEAT).
   */
 ...
 public:
  ...
  /**
   * @brief Get the pixel element size individual element (BYTE, SHORT, LONG,
   * FLOAT).
 ...
 protected:
  /**
   * @brief Protected constructor this is a way to create a simple structure
   ...
   */
  void CreateTextureCubeMap(const std::array<void*, 6> cube_map = {
