...
Win32OpenGLNone::Win32OpenGLNone(glm::uvec2 size) : size_(size) {
    ...
    if (!hwnd_dummy_) throw std::runtime_error("Couldn't create a dummy window pointer.");
    hdc_ = GetDC(hwnd_dummy_);
    if (!hdc_) throw std::runtime_error("Couldn't get an HDC.");
    GLuint pixel_format;
    ...
    if (!pixel_format) throw std::runtime_error("Couldn't initiate pixel format.");
    int ok = DescribePixelFormat(hdc_, pixel_format, sizeof(pfd), &pfd);
    if (!ok) throw std::runtime_error("Failed to describe OpenGL pixel format.");
