...
namespace frame::vulkan {
    ...
    SDLVulkanWindow::SDLVulkanWindow(glm::uvec2 size) : size_(size) {
        ...
        if (extension_count == 0) {
            throw std::runtime_error(
                fmt::format(
                    "Could not get the extension count: {}",
                    SDL_GetError()));
        }
        std::vector<const char*> extensions(extension_count);
        SDL_Vulkan_GetInstanceExtensions(sdl_window_, &extension_count,
            extensions.data());
        if (extension_count == 0) {
            throw std::runtime_error(
                fmt::format(
        ...
        vk::ApplicationInfo application_info(
            "Frame", VK_MAKE_VERSION(0, 5, 1), "SDL - Vulkan - Window",
        ...
        if (SDL_Vulkan_CreateSurface(sdl_window_, *vk_unique_instance_,
            &vk_surface) != SDL_TRUE) {
            throw std::runtime_error(
                fmt::format(
                    "Error while create vulkan surface: {}",
                    SDL_GetError()));
        }
