...
std::string Severity2String(GLenum severity) {
    switch (severity) {
        case GL_DEBUG_SEVERITY_HIGH:
            return "All OpenGL Errors, shader compilation / linking errors, "
                   "or highly - dangerous undefined behavior";
        case GL_DEBUG_SEVERITY_MEDIUM:
            return "Major performance warnings, shader compilation / linking "
...
void GLAPIENTRY MessageCallback(GLenum source, GLenum type, GLuint id, GLenum severity,
    ...
    std::string str_message =
        fmt::format("GL: {} (source [{}], type [{}], severity [{}])", message,
...
class SDLOpenGLWindow : public WindowInterface {
   public:
    SDLOpenGLWindow(const std::pair<std::uint32_t, std::uint32_t> size) : size_(size) {
    ...
    void Run() override {
        ...
        do {
            ...
            if (draw_interface_) draw_interface_->RunDraw(time.count());
            device_->Display(time.count());

            SetWindowTitle("SDL OpenGL - " + std::to_string(static_cast<int>(GetFPS(dt))));
    ...
    void SetDrawInterface(std::unique_ptr<DrawInterface>&& draw_interface) override {
...
void* InitSDLOpenGLDevice(const WindowInterface* window) {
    ...
    // GL context.
    void* gl_context = SDL_GL_CreateContext(static_cast<SDL_Window*>(window->GetWindowContext()));
    if (!gl_context) {
...
std::unique_ptr<WindowInterface> CreateSDLOpenGL(std::pair<std::uint32_t, std::uint32_t> size) {
    ...
    window->SetUniqueDevice(std::make_unique<opengl::Device>(context, size));
    return window;
}
