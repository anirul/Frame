...
TEST_F(PlyTest, CreatePlyTest) {
    ASSERT_FALSE(ply_);
    ply_ = std::make_unique<frame::file::Ply>(frame::file::FindFile("asset/model/cube.ply"));
    EXPECT_TRUE(ply_);
}

TEST_F(PlyTest, GetElementPlyTest) {
    ASSERT_FALSE(ply_);
    ply_ = std::make_unique<frame::file::Ply>(frame::file::FindFile("asset/model/apple.ply"));
    EXPECT_TRUE(ply_);
