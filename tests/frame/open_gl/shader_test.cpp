...
TEST_F(ShaderTest, CreateVertexShaderTest) {
    ...
    shader_ = std::make_unique<frame::opengl::Shader>(frame::opengl::ShaderEnum::VERTEX_SHADER);
    EXPECT_TRUE(shader_);
}

TEST_F(ShaderTest, CreateFragmentShaderTest) {
    EXPECT_FALSE(shader_);
    shader_ = std::make_unique<frame::opengl::Shader>(frame::opengl::ShaderEnum::FRAGMENT_SHADER);
    EXPECT_TRUE(shader_);
}
